<?php
/**
 * Template Name: Quiz Adoption Chaton
 * Description: Quiz pour les futurs adoptants de chaton
 */
...
$timer = get_field('adopt_chaton_quiz_timer', $quiz_post) ?: 90;
$intro_desc = get_field('adopt_chaton_quiz_intro_description', $quiz_post) ?: 'Testez vos connaissances sur les chatons avant d\'adopter votre futur compagnon !';
$intro_liste_raw = get_field('adopt_chaton_quiz_intro_liste', $quiz_post) ?: '';
...
for ($i = 1; $i <= 10; $i++) {
    $q_text = get_field('adopt_chaton_quiz_q' . $i . '_question', $quiz_post);
    if (empty($q_text)) continue;

    $reponses = array();
    $correct = array();
    foreach ($lettres as $l) {
        $reponses[$l] = get_field('adopt_chaton_quiz_q' . $i . '_reponse_' . $l, $quiz_post) ?: '';
        if (get_field('adopt_chaton_quiz_q' . $i . '_correct_' . $l, $quiz_post)) {
            $correct[] = $l;
        }
    }

    $explication = get_field('adopt_chaton_quiz_q' . $i . '_explication', $quiz_post) ?: '';
...
foreach (array('low', 'mid', 'high') as $level) {
    $results[$level] = array(
        'label' => get_field('adopt_chaton_quiz_result_' . $level . '_label', $quiz_post) ?: '',
        'message' => get_field('adopt_chaton_quiz_result_' . $level . '_message', $quiz_post) ?: '',
        'advice' => get_field('adopt_chaton_quiz_result_' . $level . '_advice', $quiz_post) ?: '',
    );
}
...
<head>
    ...
    <title>Quiz Adoption Chaton - <?php bloginfo('name'); ?></title>
...
<body class="quiz-adoption-chaton-page">
